...
BASE_SCENE_DIR = SCRIPT_DIR.parents[1]
...
layout = _load_module_from_path(
    "spot_pick_place_demo_robust_layout",
    BASE_SCENE_DIR / "spot_pick_place_demo_robust_layout.py",
)
VisualTerrainDeformer = _load_module_from_path("visual_terrain", SCRIPT_DIR / "visual_terrain.py").VisualTerrainDeformer

SCENE_PATH = SCRIPT_DIR / "spot_scene_visual_terrain.xml"
MODEL_PATH = BASE_SCENE_DIR / "runs" / "spot_go2_transfer_3p5m" / "best_eval" / "best_model.zip"
...
VISUAL_TERRAIN_RESOLUTION = 321
VISUAL_TERRAIN_OFFSET = 0.058
VISUAL_TERRAIN_FOOT_LENGTH = 0.34
VISUAL_TERRAIN_FOOT_WIDTH = 0.20
VISUAL_TERRAIN_DEPTH = 0.045
VISUAL_TERRAIN_RIDGE_HEIGHT = 0.014
VISUAL_TERRAIN_STAMP_MIN_DISTANCE = 0.18
VISUAL_TERRAIN_COLOR = wp.vec3(0.58, 0.56, 0.52)
VISUAL_TERRAIN_MATERIAL = (1.0, 0.0, 0.0)
VISUAL_TERRAIN_MESH_NAME = "/geometry/visual_lunar_terrain_mesh"
VISUAL_TERRAIN_INSTANCE_NAME = "/visual_lunar_terrain"
...
def _scene_keyframe() -> tuple[np.ndarray, np.ndarray]:
    ...
    if key is None:
        raise ValueError("Expected keyframe named 'stand' in spot_scene_visual_terrain.xml.")
    return (
...
def _terrain_source() -> tuple[np.ndarray, np.ndarray, np.ndarray, Path | None, tuple[float, float]]:
    root = ET.parse(SCENE_PATH).getroot()
    hfield = root.find(".//hfield")
    terrain = root.find(".//geom[@name='lunar_terrain']")
    if hfield is None or terrain is None:
        return (
            np.array([20.0, 20.0, 0.0, 0.0], dtype=np.float64),
    ...
    image = np.asarray(Image.open(BASE_SCENE_DIR / hfield.attrib["file"]).convert("L"), dtype=np.float64) / 255.0
    texture_path = None
    texture_repeat = (1.0, 1.0)
    material_name = terrain.attrib.get("material")
    if material_name is not None:
        material = root.find(f".//material[@name='{material_name}']")
        if material is not None:
            repeat = material.attrib.get("texrepeat")
            if repeat is not None:
                repeat_values = np.fromstring(repeat, sep=" ", dtype=np.float64)
                if repeat_values.shape[0] >= 2:
            ...
            if texture_name is not None:
                ...
                if texture is not None and texture.attrib.get("file") is not None:
                    texture_path = BASE_SCENE_DIR / texture.attrib["file"]

    return size, terrain_pos, image, texture_path, texture_repeat


def _terrain_height_at(xy: np.ndarray) -> float:
    size, terrain_pos, image, _, _ = _terrain_source()
...
def _terrain_xy_bounds() -> tuple[tuple[float, float], tuple[float, float]]:
    size, _, _, _, _ = _terrain_source()
    return (-float(size[0]), float(size[0])), (-float(size[1]), float(size[1]))


def _terrain_texture() -> Path | None:
    _, _, _, texture_path, _ = _terrain_source()
    return texture_path


def _terrain_texture_repeat() -> tuple[float, float]:
    _, _, _, _, texture_repeat = _terrain_source()
...
class SpotPickPlaceDemo:
    def __init__(self, viewer, args: argparse.Namespace):
        ...
        self.show_visual_terrain = not getattr(args, "disable_visual_terrain", False)
        self.visual_terrain = None
        self.visual_terrain_texture = None
        self.visual_terrain_depth = getattr(args, "visual_terrain_depth", VISUAL_TERRAIN_DEPTH)
        self.visual_terrain_ridge_height = getattr(args, "visual_terrain_ridge_height", VISUAL_TERRAIN_RIDGE_HEIGHT)
        self.visual_terrain_last_stamp_xy = np.full((4, 2), np.nan, dtype=np.float64)
        if self.show_visual_terrain:
            x_bounds, y_bounds = _terrain_xy_bounds()
            self.visual_terrain = VisualTerrainDeformer(
                x_bounds=x_bounds,
                y_bounds=y_bounds,
                resolution=getattr(args, "visual_terrain_resolution", VISUAL_TERRAIN_RESOLUTION),
                height_fn=_terrain_height_at,
                visual_offset=getattr(args, "visual_terrain_offset", VISUAL_TERRAIN_OFFSET),
                texture_repeat=_terrain_texture_repeat(),
                max_depression=self.visual_terrain_depth * 2.0,
                max_ridge=self.visual_terrain_ridge_height * 2.5,
            )
            self.visual_terrain_texture = _terrain_texture()
        ...
        if self.viewer is not None:
            ...
            self._configure_visual_rendering()
    ...
    def reset(self) -> None:
        ...
        self.b_align_target_root_q = None
        self.contacts_ready = False
        self.visual_terrain_last_stamp_xy.fill(np.nan)
    ...
    def _foot_contact_positions(self) -> list[tuple[int, np.ndarray]]:
        foot_to_index = {int(geom_id): idx for idx, geom_id in enumerate(self.foot_geom_ids)}
        contact_positions = []
        seen_feet = set()
        for i in range(self.solver.mj_data.ncon):
            ...
            foot_index = None
            if geom1 == self.terrain_geom_id and geom2 in foot_to_index:
                foot_index = foot_to_index[geom2]
            elif geom2 == self.terrain_geom_id and geom1 in foot_to_index:
                foot_index = foot_to_index[geom1]

            if foot_index is None or foot_index in seen_feet:
                continue

            contact_positions.append((foot_index, np.asarray(contact.pos, dtype=np.float64).copy()))
            seen_feet.add(foot_index)
        return contact_positions

    def _record_visual_terrain_contacts(self) -> None:
        if self.visual_terrain is None:
            return

        yaw = self._base_yaw()
        for foot_index, position in self._foot_contact_positions():
            xy = position[:2]
            last_xy = self.visual_terrain_last_stamp_xy[foot_index]
            if np.isfinite(last_xy).all():
                distance = float(np.linalg.norm(xy - last_xy))
                if distance < VISUAL_TERRAIN_STAMP_MIN_DISTANCE:
                    continue

            changed = self.visual_terrain.stamp_contact(
                xy,
                yaw=yaw,
                length=VISUAL_TERRAIN_FOOT_LENGTH,
                width=VISUAL_TERRAIN_FOOT_WIDTH,
                depth=self.visual_terrain_depth,
                ridge_height=self.visual_terrain_ridge_height,
            )
            if changed:
                self.visual_terrain_last_stamp_xy[foot_index] = xy

    def _log_visual_terrain(self) -> None:
        if self.visual_terrain is None:
            return

        points, indices, normals, uvs = self.visual_terrain.mesh_arrays()
        texture_enabled = 1.0 if self.visual_terrain_texture is not None else 0.0
        material = wp.vec4(*VISUAL_TERRAIN_MATERIAL, texture_enabled)
        self.viewer.log_mesh(
            VISUAL_TERRAIN_MESH_NAME,
            wp.array(points, dtype=wp.vec3),
            wp.array(indices, dtype=wp.int32),
            normals=wp.array(normals, dtype=wp.vec3),
            uvs=wp.array(uvs, dtype=wp.vec2),
            texture=str(self.visual_terrain_texture) if self.visual_terrain_texture is not None else None,
            hidden=True,
            backface_culling=False,
        )
        self.viewer.log_instances(
            VISUAL_TERRAIN_INSTANCE_NAME,
            VISUAL_TERRAIN_MESH_NAME,
            wp.array([wp.transform_identity()], dtype=wp.transform),
            wp.array([wp.vec3(1.0, 1.0, 1.0)], dtype=wp.vec3),
            wp.array([VISUAL_TERRAIN_COLOR], dtype=wp.vec3),
            wp.array([material], dtype=wp.vec4),
        )

    def _configure_visual_rendering(self) -> None:
        renderer = getattr(self.viewer, "renderer", None)
        if renderer is None:
            return

        renderer.draw_shadows = True
        renderer.shadow_radius = 1.2
        renderer.shadow_extents = 14.0
        renderer.diffuse_scale = 1.25
        renderer.specular_scale = 0.25
        renderer.ambient_sky = (0.46, 0.47, 0.50)
        renderer.ambient_ground = (0.13, 0.13, 0.14)
        renderer._light_color = (1.0, 0.96, 0.88)
        renderer._sun_direction = np.array([0.45, -0.35, -0.82], dtype=np.float64)
    ...
    def step(self) -> None:
        ...
        for _ in range(CONTROL_DECIMATION):
            ...
            self._record_visual_terrain_contacts()
            if (
...
def _create_base_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--disable-visual-terrain",
        action="store_true",
        help="Hide the USD-exportable deforming lunar terrain overlay.",
    )
    parser.add_argument(
        "--visual-terrain-resolution",
        type=int,
        default=VISUAL_TERRAIN_RESOLUTION,
        help="Resolution of the deforming visual terrain mesh in each axis.",
    )
    parser.add_argument(
        "--visual-terrain-offset",
        type=float,
        default=VISUAL_TERRAIN_OFFSET,
        help="Vertical offset [m] that keeps the visual terrain overlay above the collision hfield.",
    )
    parser.add_argument(
        "--visual-terrain-depth",
        type=float,
        default=VISUAL_TERRAIN_DEPTH,
        help="Maximum visual footstep depression depth [m].",
    )
    parser.add_argument(
        "--visual-terrain-ridge-height",
        type=float,
        default=VISUAL_TERRAIN_RIDGE_HEIGHT,
        help="Visual ridge height [m] around each footstep depression.",
    )
    parser.add_argument(
